...
export function clean(t, { punctuate = false } = {}) {
  if (!t) return '';
  let s = String(t)
    .replace(/[—–]/g, ', ')
    .replace(/ - /g, ', ')
    ...
    .trim();
  if (!s) return s;
  const first = s.search(/\p{L}/u);
  if (first !== -1) s = s.slice(0, first) + s.charAt(first).toUpperCase() + s.slice(first + 1);
  if (punctuate && !/[.?!…»:]$/.test(s)) s += '.';
  return s;
}

// Nettoyage complet d'une slide : majuscule en début de phrase + ponctuation finale
// sur les champs qui sont des phrases (titre, sous-titre, corps, items de liste).
// Point d'entrée unique utilisé partout dans l'app, quel que soit le projet/client.
export function cleanSlide(s) {
  return {
    layout: LAYOUTS.includes(s.layout) ? s.layout : (s.layout || 'text'),
    kicker: clean(s.kicker),
    title: clean(s.title, { punctuate: true }),
    subtitle: clean(s.subtitle, { punctuate: true }),
    body: clean(s.body, { punctuate: true }),
    bigNumber: (s.bigNumber || '').toString().trim(),
    quoteAuthor: clean(s.quoteAuthor),
    listItems: (s.listItems || []).map(x => clean(x, { punctuate: true })),
    elements: [],
  };
}
